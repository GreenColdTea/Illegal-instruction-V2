...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes[note % lePlayState.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "noteTweenY", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = playField.members[note % playField.length];
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float, ?library:Null<String> = null) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
		...
		Lua_helper.add_callback(lua, "makeAnimatedLuaSprite", function(tag:String, image:String, x:Float, y:Float, ?spriteType:String = "sparrow", ?library:Null<String> = null) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);
	...
	function loadFrames(spr:FlxSprite, image:String, spriteType:String, ?library:Null<String> = null)
	{
		switch(spriteType.toLowerCase().trim())
		{	
			case "texture" | "textureatlas" | "tex":
				spr.frames = AtlasFrameMaker.construct(image, null, false, library);
