...
class PlayState extends MusicBeatState
{
	...
	public var barbedWires:FlxTypedGroup<ShakableSprite>;
	var wire:ShakableSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'vista':
				...
				GameOverSubstate.endSoundName = 'chaotix-retry';
				    ...
				    camGame.filters = [staticOverlay, camFuckFilter];
                }
		...
		switch(curStage)
		{
			...
			case 'infinity-legacy':
				...
				boyfriendGroup.x -= 140;
				boyfriendGroup.y -= 175;
				dadGroup.x -= 70;
				dadGroup.y -= 175;
		...
		for(shit in 0...6) {
			var wow = shit + 1;
			wire = new ShakableSprite().loadGraphic(Paths.image('barbedWire/' + wow));
    ...
    private function strumsPositions():Void {
		if (!ClientPrefs.opponentStrums) {
			...
			opponentStrums.baseAlpha = 0.3;
			modManager.setValue('alpha', 0.7, 1);
   ...
   function set_playbackRate(value:Float):Float
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var rowArray = noteRows[swagNote.gfNote ? 2 : gottaHitNote ? 0 : 1];
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			case 'Change Character':
				...
				//why we need newCharacter var if we can just use event.value2? idk
	...
	function randomizeSongName():Void {
        ...
        switch (randomValue) {
			case 1:
				...
				scoreTxt.text = '[rAT~] ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) | sc0r3??: ' + songScore + ' | m1$: ' + songMisses + ' ' + ratingFC;
			case 2:
				songNameHUD.text = '0vVVOu';
				scoreTxt.text = '<<m15>>:' + songMisses + ' | <<RT>>:' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) | <<sCR>>:' + songScore + ' ' + ratingFC;
			case 3:
				songNameHUD.text = 'duKqfdcaXs';
				scoreTxt.text = 'rate==>' + ratingName + ' [' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%] | err0rmss=' + songMisses + ' | scr~gl!tch=' + songScore + ' ' + ratingFC;
			case 4:
				songNameHUD.text = 'iPpj5TMNW';
				scoreTxt.text = 'm1ss://=' + songMisses + ' | ~RaT=' + ratingName + ' @' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | s[CorRUPT]=' + songScore + ' ' + ratingFC;
			case 5:
				songNameHUD.text = '5JH1Bg7gRQ';
				scoreTxt.text = 'r8?=' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) || mi404:' + songMisses + ' || sc_ERR#' + songScore + ' ' + ratingFC;
			case 6:
				songNameHUD.text = 'Gkpo5g7vxm';
				scoreTxt.text = '@ms=' + songMisses + ' | ###scr=' + songScore + ' | R8=' + ratingName + ' (%' + Highscore.floorDecimal(ratingPercent * 100, 2) + ') ' + ratingFC;
			case 7:
				songNameHUD.text = 'NUSmyXyoyH';
				scoreTxt.text = 'r_8=' + ratingName + ' <' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%> | x_mss=' + songMisses + ' | ~dmp=' + songScore + ' ' + ratingFC;
			case 8:
				songNameHUD.text = '1f5VmRWPXE';
				scoreTxt.text = '[~]rt=' + ratingName + ' {' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%} | [F]sc=' + songScore + ' | [X]ms=' + songMisses + ' ' + ratingFC;
			case 9:
				songNameHUD.text = 'MLioiJtZX4';
				scoreTxt.text = ':: OUT:' + ratingName + ' @' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% :: ###' + songScore + ' :: ERR:' + songMisses + ' ' + ratingFC;
			case 10:
				songNameHUD.text = 'WPvSwx9e5d';
				scoreTxt.text = 'ra###:' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) | s????:' + songScore + ' | m!?s:' + songMisses + ' ' + ratingFC;
			case 11:
				songNameHUD.text = 'E0U0xZuJ8p';
				scoreTxt.text = 'rtng>>' + ratingName + ' <' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%> //miss~' + songMisses + ' //scr=' + songScore + ' ' + ratingFC;
			case 12:
				songNameHUD.text = 'd9f8cj1Rs3';
				scoreTxt.text = 'R-T ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) || M!$$ ' + songMisses + ' || SC>>> ' + songScore + ' ' + ratingFC;
			case 13:
				songNameHUD.text = 'ZLyE8jMV62';
				scoreTxt.text = 'GLRT:' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%) / ERR:' + songMisses + ' / D:Sc=' + songScore + ' ' + ratingFC;
			case 14:
				songNameHUD.text = '9j3fOVV9Sw';
				scoreTxt.text = 'r%bug=' + ratingName + ' [' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%] // m_404=' + songMisses + ' // s_x=' + songScore + ' ' + ratingFC;
			case 15:
				songNameHUD.text = 'Tails!!!';
				scoreTxt.text = '!rate=' + ratingName + ' <' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%> | %scr;' + songScore + ' | NUllm1ss:' + songMisses + ' ' + ratingFC;
		}
    }
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (showCombo)
		{
			insert(members.indexOf(playFields), comboSpr);
		}
		if (!ClientPrefs.comboStacking)
		{
		...
		for (i in seperatedScore)
		{
			...
			//if (combo >= 10 || combo == 0)
			if(showComboNum) insert(members.indexOf(rating), numScore);
	...
	function updateWires() {
		inline function cancelDisappearTween(wire:ShakableSprite) {
		...
		for (wireIdx in 0...barbedWires.members.length) {
            var wire:ShakableSprite = cast barbedWires.members[wireIdx];
		    if (wire == null) continue; // for da best time
			wire.screenCenter();
			var inUse:Bool = wire.extraInfo.get("inUse");

            if ((wireIdx + 1) <= hexes) {
                if (!inUse) {
                    cancelDisappearTween(wire);
                    wire.alpha = 1;
			   ...
			   if (curStage == "entrance")
			   {
				switch (curStep)
				{
					...
					case 768:
                        ...
                        if (ClientPrefs.shaders) {
						    camGame.filters = [barrelDistortionFilter];
						    camHUD.filters = [barrelDistortionFilter];
	                    }
					case 784:
		                if (ClientPrefs.shaders) {
					...
					case 1584:
						...
						camGame.filters = [new ShaderFilter(gray.shader)];
						holyFuckStopZoomin = false;
						wowZoomin = false;
						defaultZoomin = false;
					case 1736:
					...
					case 1870:
                        if (ClientPrefs.shaders && !ClientPrefs.lowQuality) {
						    camGame.filters = [camGlitchFilter, barrelDistortionFilter];
						    camHUD.filters = [camGlitchFilter, barrelDistortionFilter];
					...
					switch (curStep)
					{
						...
						case 924:
							...
							FlxTween.tween(whiteFuck, {alpha: 1}, 6, {ease: FlxEase.cubeInOut, onComplete: function(twn:FlxTween)
								{
									deleteSprite(fucklesFGPixelLegacy);
									deleteSprite(fucklesBGPixelLegacy);
				...
				switch (curStep)
				{
					...
					case 2336:
						...
						if(ClientPrefs.flashing && ClientPrefs.shaders && !ClientPrefs.lowQuality) {
							camGame.filters = [camGlitchFilter, camFuckFilter, staticOverlay];
							camHUD.filters = [camGlitchFilter, camFuckFilter, staticOverlay];
						}
                        if (ClientPrefs.shaders) {
	...
	override function beatHit()
	{
		...
		if (curBeat % 2 == 0 && curStage == 'vista' && !fuckedBar)
		{
		...
		if (curBeat % 4 == 0 && curStage == 'vista' && !fuckedBar)
		{
   ...
   override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			if (defaultZoomin && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		        {
		   	    FlxG.camera.zoom += 0.0242 * camZoomingMult;
	...
	function iShouldKickUrFuckinAss(die:Int)
		{
			switch (die)
			{
				case 1:
					...
					deleteSprite(entranceBG);
					deleteSprite(entranceClock);
					deleteSprite(entranceIdk);
					deleteSprite(entranceFloor);
					deleteSprite(entranceOver);
			        Paths.clearUnusedMemory();
				...
				case 2:
					...
					deleteSprite(vistaFlower);
					deleteSprite(vistaTree);
					deleteSprite(vistaBush);
					deleteSprite(vistaGrass);
					deleteSprite(vistaFloor);
					deleteSprite(vistaBG);

					deleteSprite(amyBop);
					deleteSprite(vectorBop);
					deleteSprite(charmyBop);
					deleteSprite(espioBop);
					deleteSprite(mightyBop);
					deleteSprite(knuxBop);
	...
	function deleteSprite(spr:FlxSprite)
	{
